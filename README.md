...
- файл `.env`
    ...
    Файл `.env` в корне проекта подгружается автоматически (через `dotenv`) и заполняет переменные окружения. Используются те же имена с префиксом `BI_`:
    ```
    BI_SERVER=http://project.luxmsbi.com/
    BI_USERNAME=admin
    BI_PASSWORD=secret
    ```
    Файл занесён в `.gitignore` и не должен попадать в git — в нём, как правило, хранятся секреты. Реальные переменные окружения и опции командной строки имеют приоритет над `.env`.
...
### Запуск с браузерной сессией БЛИЦ

После входа через БЛИЦ можно использовать значение cookie `LuxmsBI-User-Session`
в локальном `authConfig.json` (файл исключён из Git):
...
Запуск: `yarn start`. Сессия имеет приоритет над
логином/паролем и JWT. Сервер проверяет её через `/api/auth/check`, передаёт
в проксируемых запросах и слушает только `127.0.0.1`. Редиректы при проверке
не выполняются; браузерная сессия не завершается при запуске.
После истечения сессии обновите cookie в конфиге и перезапустите процесс.
Сессия также используется командами `yarn pull` и `yarn push`; для загрузки
ресурсов у пользователя должны быть соответствующие права.

Для сервера разработки с некорректным сертификатом можно явно поставить
`insecureSessionTls: true`: это отключает проверку сертификата у запросов
проверки сессии и синхронизации с настроенным BI-сервером. Dev-прокси уже использует `secure: false`.

Секрет можно передать через `BI_SESSION` или `--session=...`; для исключения TLS
используйте `BI_INSECURE_SESSION_TLS=true` или `--insecure-session-tls=true`.
Значение `false` сохраняет проверку сертификата. Cookie не выводится в журнал.
